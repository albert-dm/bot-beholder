...
class QuickReplyAnswer extends Component {
    ...
    handleChange = e => {
        ...
        let newAnswer = { ...this.props.answer };
        newAnswer[name] = value;
        this.updateAnswer(newAnswer);
    }
    handleItemChange = e => {
        const { name, value } = e.target;
        let newItemForm = { ...this.state.itemForm };
                        ...
                        if (this.props.editing) {
                            return (
                                <React.Fragment>
                                    <div className="card bubble">
                                        <div className="wrap">
                                            <h4>QuickReply</h4>
                                            <textarea onChange={this.handleChange} name="text" value={this.props.answer.text} />
                                        </div>
                                    </div>
                                    <div className="chips-container">
                                        {this.props.answer.items.map(
                                            (item, index) => (<QuickReplyItem item={item} index={index} setItem={this.setItem} key={index} delete={() => this.deleteItem(index)} />)
                                        )}
                                    </div>
                                    <div className="card" style={{ marginLeft: 'auto', marginRight: 'auto' }}>
                                        <div className="wrap" >
                                            <h4>Novo item</h4>
                                            <p><b>Título:</b> <input onChange={this.handleItemChange} type='text' name="title" value={this.state.itemForm.title} /></p>
                                            <p><b>Payload:</b> <input onChange={this.handleItemChange} type='text' name="payload" value={this.state.itemForm.payload} /></p>
                            ...
                            return (
                                <React.Fragment>
                                    ...
                                    <div className="chips-container">
                                        {this.props.answer.items.map(
                                            (item, index) => (<p key={index} className="chip">{item.title} </p>)
                                        )}
                                    </div>
                                </React.Fragment>
                            )
